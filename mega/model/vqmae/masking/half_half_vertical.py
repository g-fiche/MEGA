...
def half_indexes(size: int):
    forward_indexes = np.arange(size).reshape(int(math.sqrt(size)), int(math.sqrt(size)))
    rand = random.randint(0, 1)
    if rand == 0:
        forward_indexes = np.concatenate((forward_indexes[0:8, :], forward_indexes[8:, :])).reshape(-1)
    else:
        forward_indexes = np.concatenate((forward_indexes[8:, :], forward_indexes[0:8, :])).reshape(-1)
...
class PatchShuffleHalfHalfVertical(torch.nn.Module):
    ...
    def forward(self, patches: torch.Tensor):
        ...
        indexes = [half_indexes(T) for _ in range(B)]
